...
class MemoryCache(Cache):
    ...
    def add_to_history(self, id, field, value):
        history = self.cache.setdefault(id, {}).setdefault("question_history", [])
        history.append({field: value})
        self.cache[id]["question_history"] = history

    def get_history(self, id):
        if id in self.cache:
            return self.cache[id].get("question_history", [])